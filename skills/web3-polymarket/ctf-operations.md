...
|-------|-------------|-----------|
| **Yes** | $1.00 USDC | Event occurs |
| **No** | $1.00 USDC | Event does not occur |

Every Yes/No pair is backed by exactly $1.00 USDC locked in the CTF contract.

## Split

Convert USDC into a full set of outcome tokens.
...
2. USDC approval for CTF contract
3. Condition ID of the market
...
| `amount` | uint256 | Amount of USDC to split |

## Merge

Convert a full set of outcome tokens back to USDC. Inverse of split.
...
2. Condition ID
3. No approval needed — you already hold the tokens
...
Exchange winning tokens for USDC after market resolution.
...
| Parameter | Type | Value |
|-----------|------|-------|
| `collateralToken` | address | USDC address |
...
|-----------|-------------------|-------|
| Buy order (standard) | CTF Exchange | USDC |
| Sell order (standard) | CTF Exchange | Conditional tokens |
| Buy order (neg risk) | Neg Risk CTF Exchange | USDC |
...
3. `getPositionId(collateralToken, collectionId)` — combines USDC address with collection
